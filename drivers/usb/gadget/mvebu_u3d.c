...
int eps_init(struct mvc2 *cp)
{
	...
	if (!cp->setup_buf) {
		dev_err(cp->dev, "failed to allocate setup buffer\n");
		ret = -ENOMEM;
	}
					      ...
					      gfp_t gfp_flags)
{
	...
	if (!req) {
		dev_err(cp->dev, "fail to alloc mem for req\n");
		return NULL;
	}
		   ...
		   unsigned offset, unsigned *last)
{
	 ...
	 * complete gradually.
	 * DMA engine would never cache accross two segments
...
#define USB3D_MAX_WINDOWS	4
#define USB3D_WIN_CTRL(w)	(0x0 + ((w) * 8))
#define USB3D_WIN_BASE(w)	(0x4 + ((w) * 8))

static void mv_usb3d_conf_mbus_windows(void __iomem *base,
				       const struct mbus_dram_target_info *dram)
{
	int win;

	/* Clear all existing windows */
	for (win = 0; win < USB3D_MAX_WINDOWS; win++) {
		writel(0, base + USB3D_WIN_CTRL(win));
		writel(0, base + USB3D_WIN_BASE(win));
	}
	...
	for (win = 0; win < dram->num_cs; win++) {
		...
		writel(((cs->size - 1) & 0xffff0000) | (cs->mbus_attr << 8) |
		       ...
		       base + USB3D_WIN_CTRL(win));

		writel((cs->base & 0xffff0000), base + USB3D_WIN_BASE(win));
	}
}
...
static int mvc2_probe(struct platform_device *pdev)
{
	...
	if (!cp) {
		dev_err(&pdev->dev, "failed to allocate memory for usb3\n");
		return -ENOMEM;
	}
	...
	if (res) {
		win_base = devm_ioremap_resource(&pdev->dev, res);
		if (!win_base) {
			ret = -ENOMEM;
			goto err_clk;
		}
		...
		mv_usb3d_conf_mbus_windows(win_base, dram);
	} else {
		ret = -ENODEV;
		goto err_clk;
	}
	...
	phy_base = devm_request_and_ioremap(&pdev->dev, res);
	if (!phy_base) {
...
static int mvc2_remove(struct platform_device *dev)
{
	...
	cp = (struct mvc2 *)dev_get_drvdata(&dev->dev);
	mvc2_connect(cp, 0);
